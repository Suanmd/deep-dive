...
from deep_dive.constants import LOWQ_DOMAINS, TAG_ERR
...
_CN_HOST_HINTS = (
    ".cn", "baidu.com", "sogou.com", "qq.com", "weibo.com",
    "toutiao.com", "sohu.com", "163.com", "bilibili.com", "zhihu.com",
    "csdn.net", "cnblogs.com", "tsinghua.edu.cn",
    "wainao.me", "stdaily.com", "cctv.com", "qidian.com",
    "hao86.com", "renrendoc.com", "vocus.cc", "amazon.co.jp",
)
...
def build_report(
    *,
    ...
    for u, m in url_meta.items():
        ...
        if m.status == FetchStatus.SUCCESS:
            if m.chars < min_chars:
                m = FetchResult(
                    url=m.url, status=m.status, title=m.title, chars=m.chars,
                    html_path=m.html_path, txt_path=m.txt_path, error=m.error,
                    source_task=m.source_task, query_index=m.query_index,
                    extra={**m.extra, "render_quality": "low", "lowq_reason": "low_chars"},
                )
                n_lowq += 1
                ...
                m = FetchResult(
                    ...
                    html_path=m.html_path, txt_path=m.txt_path, error=m.error,
                    source_task=m.source_task, query_index=m.query_index,
                    extra={**m.extra, "render_quality": "ok"},
                )
    ...
    f = open(report_path, "w", encoding="utf-8", buffering=1024 * 16)
    ...
    if plan is not None:
        lines += [
            "## 调研策略 (LLM Plan)",
            "",
            f"本 run 由 LLM 生成的 ResearchPlan 驱动。决策依据如下：",
            "",
            f"- **query**: `{plan.query}`",
    ...
    for i, (row, res) in enumerate(zip(matrix, task_results), 1):
    ...
    for idx, (u, m) in enumerate(items, 1):
        ...
        if txt_file is not None:
            ...
            if not txt_file_path.exists():
                ...
                if candidate_name in txt_paths_by_name:
                    txt_file_path = txt_paths_by_name[candidate_name]
                else:
                    txt_file_path = None
        else:
    ...
    if n_lowq > 0 or n_blacklisted > 0:
        lowq_items = [(u, m) for u, m in url_meta_rendered.items() if m.extra.get("render_quality") == "low"]
    ...
    lines += ["## 4. 元数据", "", "```json", json.dumps(summary_block, ensure_ascii=False, indent=2), "```", ""]
    w(lines)
