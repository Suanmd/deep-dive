...
implemented inline in the previous ``deep-search`` crawler. We pull
it into a class so:
...
from deep_dive.crawler.blacklist import is_black_domain, is_baidu_domain, is_spam_domain
from deep_dive.crawler.cookies import Cookie, count_loaded, load_cookies, match_cookies_to_url
from deep_dive.crawler.extraction import extract_main_text, extract_title, looks_like_block_page
from deep_dive.crawler.fetchers.base import Fetcher
from deep_dive.crawler.fetchers.cloudscraper import CloudScraperFetcher
...
class _FetchMetadata:
    """In-memory representation of the ``metadata.json`` we write per task."""
...
class CrawlPipeline:
    ...
    async def _process_one(
        ...
        *,
        ...
        if is_spam_domain(url):
            return FetchResult(
                url=url, status=FetchStatus.SKIPPED,
                error="spam_domain", source_task=source_task, query_index=query_index,
            )
        if is_black_domain(url):
            # Try cloudscraper instead of skipping outright (legacy behavior)
            if self.fallback_fetcher:
                return await self._process_via_fallback(url, source_task, query_index)
            return FetchResult(
                url=url, status=FetchStatus.SKIPPED,
                error="cf_blacklist", source_task=source_task, query_index=query_index,
            )
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if self.fallback_fetcher:
                    return await self._process_via_fallback(url, source_task, query_index)
                return FetchResult(
                    url=url, status=FetchStatus.FAILED,
                    title=title or "", error=f"{type(e).__name__}: {e}"[:200],
                    source_task=source_task, query_index=query_index,
                )

        if not html:
            return FetchResult(
                url=url, status=FetchStatus.FAILED,
                title=title or "", error="empty html",
                source_task=source_task, query_index=query_index,
            )
        ...
        if looks_like_block_page(text):
            ...
            return FetchResult(
                ...
                title=title, error="block_page_detected",
                source_task=source_task, query_index=query_index,
            )
        ...
        # 4. Relevance check
        if (
            self.config.enable_relevance_check
            and text
            and self.config.main_query
        ):
            try:
                if is_query_irrelevant(text, self.config.main_query):
                    try:
                        ...
                        pass
                    return FetchResult(
                        url=url, status=FetchStatus.IRRELEVANT,
                        title=title, chars=len(text),
                        html_path=html_path, txt_path=None,
        ...
        return FetchResult(
            ...
            title=title, chars=len(text),
            html_path=html_path, txt_path=txt_path,
            source_task=source_task, query_index=query_index,
        )

    async def _process_via_fallback(
        self, url: str, source_task: str, query_index: int,
    ) -> FetchResult:
        if self.fallback_fetcher is None:
            return FetchResult(
                url=url, status=FetchStatus.SKIPPED,
                error="no_fallback", source_task=source_task, query_index=query_index,
            )
        ...
        if not html:
            return FetchResult(
                url=url, status=FetchStatus.FAILED,
                error="fallback_empty", source_task=source_task, query_index=query_index,
            )
        ...
        if looks_like_block_page(text):
            return FetchResult(
                url=url, status=FetchStatus.BLOCKED,
                title=title, error="block_page_after_fallback",
                source_task=source_task, query_index=query_index,
            )
    ...
    def _record_metadata(self, result: FetchResult) -> None:
        entry: dict[str, Any] = asdict(result) if hasattr(result, "__dataclass_fields__") else {
            "url": result.url,
