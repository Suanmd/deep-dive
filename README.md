...
[English version](#english-version) · [English version below]

---

## ✨ 核心特性

- **多角度搜索矩阵** — 5 种查询模板（人文 / 技术 / 学术 / 新闻 / 商业）× 3 档深度（quick / normal / full）× 5 种视角变体（original / refined / critique / academic / primary / comparative）
- **双引擎策略** — MMX（主，需安装 mmx CLI）+ Tavily（兜底，自动 KEY1 → KEY2 fallback）
...
# 可选：备份 key（v1.0 支持自动 KEY1 → KEY2 fallback）
export TAVILY_API_KEY_BACKUP="tvly-..."
```
...
- `variants`（5 种视角查询变体）— 自动用 query + 语种后缀生成
...
```
        ...
        ▼                               ▼                               ▼
┌────────────────┐            ┌─────────────────┐            ┌────────────────┐
│   Orchestrator │            │  Auto-detect +   │           │   Reporting     │
│ search matrix  │            │  smart defaults   │           │  - builder.py   │
│ + parallel     │            │ (kind / lang /    │           │  - capy_summary │
│ dispatch       │            │  variants / thresh)│           └────────────────┘
└────────┬───────┘            └─────────────────┘
         │
         ▼
┌────────────────────────────────────────────────────────────────────────┐
│                       Pipeline (per task)                              │
│                                                                        │
│   Search ──► Filter ──► Fetch ──► Extract ──► Relevance check          │
│   (MMX/Tavily) (smart) (Playwright/cloudscraper) (trafilatura)         │
└────────────────────────────────────────────────────────────────────────┘
```
...
> Deep research engine for agents. Plan-driven multi-perspective search +
> site targeting + full-text scraping + dedup aggregation + structured
> Markdown report.

A clean open-source re-implementation. All user-tunable values live in
`config/`, not in code. SKILLS auto-handles kind / language / variants /
threshold / target_sites when no `--plan` is supplied.
...
export TAVILY_API_KEY="***"
deep-dive --query "<topic>" --depth normal
```

See [docs/usage.md](docs/usage.md) for the full CLI cookbook and
