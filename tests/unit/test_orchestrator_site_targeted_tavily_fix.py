...
class _IgnoreSiteEngine(SearchEngine):
    ...
    def _raw_search(self, query, topk):
        ...
        return [
            SearchHit(url=u, title=f"mmx-{i}", engine=self.name)
...
class _HonorSiteEngine(SearchEngine):
    ...
    def _raw_search(self, query, topk):
        self.search_calls.append(query)
        return [
            SearchHit(url=u, title=f"tavily-{i}", engine=self.name)
            for i, u in enumerate(self.urls[:topk])
        ]
...
class TestSiteTargetedPrefersTavily:
    ...
    def test_site_targeted_uses_tavily_primary_when_available(self, tmp_path):
        ...
        mmx_garbage = _IgnoreSiteEngine([
            f"https://wrong-domain.com/{i}" for i in range(9)
        ])
        ...
        result = _run_one_task(
            ...
            config=cfg, cookies_map={}, main_query="test",  # match _StubFetcher HTML for relevance
        ...
        assert result.extra["engine"] == "tavily", (
            f"site-targeted primary must be Tavily (got "
            f"{result.extra['engine']})"
        )
        # Tavily got the site: query first (i.e., was primary, not fallback).
        assert any(
            "site:arxiv.org" in q for q in tavily_correct.search_calls
        ...
        assert result.url_count >= 5, (
            f"expected ≥5 successful fetches (Tavily's arxiv hits), "
            f"got {result.url_count}"
        )
    ...
    def test_site_targeted_mm_x_fallback_when_tavily_quota(self, tmp_path):
        ...
        quota_tavily = _HonorSiteEngine([])  # returns nothing — simulates quota-fail recovery
        ...
        result = _run_one_task(
            ...
            config=cfg, cookies_map={}, main_query="Attention Is All You Need",
            fetcher_classes={"primary": _StubFetcher, "fallback": _StubFetcher},
        )
        ...
        assert result.extra.get("degraded_to") == "mmx", (
            f"Tavily quota must degrade to MMX (got degraded_to="
            f"{result.extra.get('degraded_to')})"
        )
        # MMX was called as fallback.
        assert len(mmx_garbage.search_calls) >= 1, (
            "MMX must be called as fallback after Tavily quota"
        )
    ...
    def test_explicit_mm_x_preserves_user_intent_for_site_targeted(self, tmp_path):
        ...
        assert tavily_correct.search_calls == [], (
            "Tavily must NOT be called when user explicitly chose MMX"
        )
    ...
    def test_non_site_targeted_uses_mmx_primary_regression(self, tmp_path):
        ...
        """
        mmx_partial = _IgnoreSiteEngine([
            f"https://example.com/{i}" for i in range(3)
        ])
        tavily_supplement = _HonorSiteEngine([
            f"https://example.com/{i}" for i in range(3, 10)
        ])
        cfg = _make_config(tmp_path)
        from deep_dive.orchestrator import MatrixRow
        row = MatrixRow(
        ...
        assert result.status == TaskStatus.SUCCESS
        # MMX primary, Tavily fallback (legacy behaviour preserved).
        assert result.extra["engine"] == "mmx"
    ...
    def test_site_targeted_no_tavily_available_still_fails(self, tmp_path):
        ...
        falls back to MMX (legacy behaviour). The task will likely fail
        ...
        # This is the legacy "no Tavily = MMX only" behaviour, preserved.
